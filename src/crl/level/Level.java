...
public class Level implements FOVMap, Serializable {
	...
	public void removeItemFrom(Item what, Position where) {
		ArrayList<MenuItem> stack = items.get(where.toString());
		if (stack != null) {
			stack.remove(what);
			if (stack.isEmpty())
	...
	public int getFrostAt(Position where) {
		Counter x = frosts.get(where.toString());
		if (x != null)
			return 1;
		else
			return 0;
	...
	public boolean isItemPlaceable(Position where) {
		return isWalkable(where) && getFeatureAt(where) == null && !getMapCell(where).isShallowWater()
	...
	public void respawn() {
		...
		case MonsterSpawnInfo.BORDER:
			for (int i = 0; i < 30; i++) {
				...
				validate(nearPlayer);
				if (getMapCell(nearPlayer) == null || getMapCell(nearPlayer).isEthereal()
						|| ((getMapCell(nearPlayer).isWater() || getMapCell(nearPlayer).isShallowWater())
								&& !monster.canSwim())
	...
	public void spawnTreasure() {
		...
		while (true) {
			...
			if (getMapCell(nearPlayer) == null)
				continue;
			if (getMapCell(nearPlayer).getHeight() == 0 && !getMapCell(nearPlayer).isWater()
					&& !getMapCell(nearPlayer).isShallowWater() && !getMapCell(nearPlayer).isSolid()) {
				String treasure = "";
	...
	public void anihilate() {
		...
		for (Feature mound : mounds)
			features.removeFeature(mound);
		monsters.removeAll();
	...
	public void anihilateMonsters() {
		for (int i = 0; i < monsters.size(); i++) {
			if (boss != null && monsters.get(i) == boss)
				continue;
			if (monsters.get(i).getID().equals("IGOR"))
				continue;
			if (!(monsters.get(i) instanceof NPC)) {
				dispatcher.removeActor(monsters.get(i));
	...
	public void destroyCandles() {
		...
		for (Feature candle : candles)
			destroyFeature(candle);
	}
	...
	public static final int MORNING = 1, NOON = 2, AFTERNOON = 3, DUSK = 4, NIGHT = 5, DAWN = 6;
	...
	public void savePop() {
		tempActors = new ArrayList<>(dispatcher.getActors());
	}
	...
	public void setSeen(int x, int y) {
		...
		if (Position.distance(tempSeen, player.getPosition()) <= player.getSightRange()
				|| lit[tempSeen.z][tempSeen.x][tempSeen.y]) {
