...
public class GFXDisplay extends Display {
	...
	private void initProperties(Properties p) {
		...
		try {
			...
			IMG_MAPMARKER = PropertyFilters.getImage(p.getProperty("IMG_MAPMARKER"),
					p.getProperty("IMG_MAPMARKER_BOUNDS"));
	...
	public GFXDisplay(SwingSystemInterface si, Properties p, GFXConfiguration configuration) {
		...
		try {
			...
			addornedTextArea = new AddornedBorderTextArea(b1, b2, b3, b4, new Color(187, 161, 80),
			...
			gfxChatBox = new GFXChatBox(b1, b2, b3, b4, new Color(187, 161, 80), new Color(92, 78, 36), 32, 32);
	...
	public int showTitleScreen() {
		...
		si.printAtPixelCentered(middlePoint, (int) (530 * scale), "'CastleVania' is a trademark of Konami Corporation.",
				GFXDisplay.COLOR_BOLD);
		si.printAtPixelCentered(middlePoint, (int) (555 * scale),
				"CastlevaniaRL v" + Game.getVersion() + ", Developed by Santiago Zapata 2005-2007, 2010, 2024",
				Color.WHITE);
		si.printAtPixelCentered(middlePoint, (int) (570 * scale), "Artwork by Christopher Barrett, 2006-2007",
				Color.WHITE);
		si.printAtPixelCentered(middlePoint, (int) (585 * scale),
				"Midi Tracks by Jorge E. Fuentes, JiLost, Nicholas and Tom Kim", Color.WHITE);
		...
		while (true) {
			...
			while (x.code != CharKey.A && x.code != CharKey.a && x.code != CharKey.B && x.code != CharKey.b
					&& x.code != CharKey.C && x.code != CharKey.c && x.code != CharKey.D && x.code != CharKey.d
					&& x.code != CharKey.E && x.code != CharKey.e && x.code != CharKey.G && x.code != CharKey.g
					&& x.code != CharKey.F && x.code != CharKey.f && x.code != CharKey.UARROW
					&& x.code != CharKey.DARROW && x.code != CharKey.SPACE && x.code != CharKey.ENTER)
				x = si.inkey();
			switch (x.code) {
			case CharKey.A:
			case CharKey.a:
	...
	public void showIntro(Player player) {
		...
		t1.setText("In the year of 1691, a dark castle emerges from the cursed soils of the plains of Transylvannia. "
				...
				+ " stands on a forest near the town of Petra and the cursed castle; " + player.getPlot2()
	...
	public boolean showResumeScreen(Player player) {
		...
		t1.setForeground(Color.WHITE);
		t1.setText("  ...And so it was that " + player.getDescription() + ", " + gsi.getDeathString() + " on the "
				+ player.getLevel().getDescription() + "...\n\n" + heshe + " scored " + player.getScore()
				+ " points and earned " + player.getGold() + " gold \n\n" + heshe + " survived for " + gsi.getTurns()
	...
	public void showHiscores(HiScore[] scores) {
		...
		for (int i = 0; i < scores.length; i++) {
			...
			si.print(43 + leftMargin, 9 + i, scores[i].getDeathString() + " on level " + scores[i].getDeathLevel(),
					Color.GRAY);
	...
	public void showChat(String chatID, Game game) {
		...
		for (int i = 0; i < chat.getConversations(); i++) {
			...
			gfxChatBox.set(image, ScriptUtil.replace(marks, replacements, chat.getName(i)),
					ScriptUtil.replace(marks, replacements, chat.getConversation(i)));
