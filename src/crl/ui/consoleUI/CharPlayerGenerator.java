...
public class CharPlayerGenerator extends PlayerGenerator {
	...
	public Player generatePlayer() {
		...
		while (x.code != CharKey.M && x.code != CharKey.m && x.code != CharKey.F && x.code != CharKey.f)
			x = si.inkey();
		int sex = determineSex(x);

		CharAppearance[] apps = determineApps();
		...
		while (true) {
			...
			si.refresh();
			while (x.code != CharKey.UARROW && x.code != CharKey.DARROW && x.code != CharKey.SPACE
	...
	private CharAppearance[] determineApps() {
	...
	private static int determineSex(CharKey x) {
		int sex = 0;
