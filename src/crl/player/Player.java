...
public class Player extends Actor {
	...
	public static final Advancement ADV_MARS = new AdvMars();
	public static final Advancement ADV_JUPITER = new AdvJupiter();
	public static final Advancement ADV_SATURN = new AdvSaturn();
	public static final Advancement ADV_URANUS = new AdvUranus();
	public static final Advancement ADV_NEPTUNE = new AdvNeptune();
	public static final Advancement ADV_PLUTO = new AdvPluto();

	public static final Advancement[] ALL_ADVANCEMENTS = new Advancement[] { ADV_MERCURY, ADV_VENUS, ADV_TERRA,
			ADV_MARS, ADV_JUPITER, ADV_SATURN, ADV_URANUS, ADV_NEPTUNE, ADV_PLUTO };
	...
	public static final String INCREMENT_HITS = "hits", INCREMENT_HEARTS = "hearts", INCREMENT_ATTACK = "attack",
			INCREMENT_COMBAT = "combat", INCREMENT_INVOKATION = "invok", INCREMENT_SPEED = "speed",
			INCREMENT_SOUL = "soul", INCREMENT_CARRYING = "carry", INCREMENT_DEFENSE = "defense",
	...
	public static void initializeWhips(String leatherWhip, String chainWhip, String vampireKiller, String thornWhip,
	...
	public void increaseWeaponSkill(String category) {
		...
		if (c.getCount() > s.getCount() * 80 + 10) {
			...
			if (s.getCount() == 9) {
				...
				UserInterface.getUI().showImportantMessage("You become better with "
						+ ItemDefinition.getCategoryDescription(category) + ". Press Space to continue.");
				s.increase();
			}
		}
	}
	...
	private int blockDirection1;
	private int blockDirection2;
	...
	public String getEquipedWeaponDescription() {
		if (weapon != null)
			return (weapon.hasCounter(Consts.C_WEAPON_ENCHANTMENT) ? "Enchanted " : "")
	...
	public void landOn(Position destinationPoint, boolean step) {
		...
		Vector<MenuItem> destinationItems = level.getItemsAt(destinationPoint);
		...
		if (destinationFeatures != null) {
			for (Feature feature : destinationFeatures) {
				...
				if (destinationFeature.getID().equals("TELEPORT")) {
					if (getGold() > 1000) {
						...
						if (UserInterface.getUI().prompt()) {
							if (getHostage() != null) {
								UserInterface.getUI()
										.showMessage("Abandon " + getHostage().getDescription() + "? [Y/N]");
								if (UserInterface.getUI().prompt()) {
									abandonHostage();
								} else {
									return;
	...
	public Vector<MenuItem> getAvailableSkills() {
		...
		if (weapon != null && weapon.getWeaponCategory().equals(ItemDefinition.CAT_RINGS)
				&& weaponSkill(ItemDefinition.CAT_RINGS) == 10)
			availableSkills.add(skills.get("SPINNING_SLICE"));
		if (weapon != null && weapon.getWeaponCategory().equals(ItemDefinition.CAT_WHIPS)
				&& weaponSkill(ItemDefinition.CAT_WHIPS) == 10)
			availableSkills.add(skills.get("WHIRLWIND_WHIP"));
		if (weapon != null && weapon.getWeaponCategory().equals(ItemDefinition.CAT_STAVES)
				&& weaponSkill(ItemDefinition.CAT_STAVES) == 10)
			availableSkills.add(skills.get("ENERGY_BEAM"));
		if (weapon != null && weapon.getWeaponCategory().equals(ItemDefinition.CAT_SWORDS)
				&& weaponSkill(ItemDefinition.CAT_SWORDS) == 10)
			availableSkills.add(skills.get("FINAL_SLASH"));
		if ((weapon == null || weapon.getWeaponCategory().equals(ItemDefinition.CAT_UNARMED))
				&& weaponSkill(ItemDefinition.CAT_UNARMED) == 10)
			availableSkills.add(skills.get("TIGER_CLAW"));
		if (weapon != null && weapon.getWeaponCategory().equals(ItemDefinition.CAT_PISTOLS)
	...
	public static final int KEYINMINENT = 3;

	public static final int EVT_FO23RWARD = 7;
	public static final int EVT_RE23TURN = 8;
	public static final int EVT_MERCHANT = 9;
	public static final int EVT_SMASHED = 10;
	public static final int EVT_CHAT = 11;
	public static final int EVT_LEVELUP = 12;
	public static final int EVT_NEXT_LEVEL_DEPRECATED = 13;
	public static final int EVT_BACK_LEVEL_DEPRECATED = 14;
	public static final int EVT_GOTO_LEVEL = 15;
	public static final int EVT_FORWARDTIME = 16;
	public static final int EVT_INN = 17;

	public static final int MALE = 1;
	public static final int FEMALE = 2;

	public static final int CLASS_VAMPIREKILLER = 0;
	public static final int CLASS_RENEGADE = 1;
	public static final int CLASS_VANQUISHER = 2;
	public static final int CLASS_INVOKER = 3;
	public static final int CLASS_MANBEAST = 4;
	public static final int CLASS_KNIGHT = 5;

	public static final int DAMAGE_MORPHED_WITH_STRONG_ARMOR = 0;
	public static final int DAMAGE_WALKED_ON_LAVA = 1;
	public static final int DAMAGE_USING_ITEM = 2;
	public static final int DAMAGE_POISON = 3;
	public static final int DAMAGE_JINX = 4;

	public static final String STATUS_STUN = "STUN";
	public static final String STATUS_POISON = "POISON";
	public static final String STATUS_PETRIFY = "PETRIFY";
	public static final String STATUS_FAINTED = "FAINTED";
	...
	public static final int SACRED_CRYSTAL = 6;
	public static final int SACRED_FIST = 7;
	public static final int SACRED_REBOUND = 8;

	private static final Action[] MYSTIC_ACTIONS = new Action[] { new Dagger(), new Axe(), new Holy(), new Stopwatch(),
			new Cross(), new Bible(), new BlastCrystal(), new SacredFist(), new Rebound() };
	...
	public int getSightRange() {
		int base = baseSightRange + (level.isDay() ? 3 : 0)
				+ (level.getMapCell(getPosition()) != null ? level.getMapCell(getPosition()).getHeight() > 0 ? 1 : 0
						: 0)
				+ (hasCounter(Consts.C_MAGICLIGHT) ? 3 : 0) + (hasCounter("LIGHT") ? 3 : 0);
	...
	public static Advancement[][] STATADVANCEMENTS = new Advancement[][] { { ADV_VENUS, ADV_PLUTO, ADV_SATURN },
			{ ADV_MARS, ADV_MERCURY, ADV_URANUS }, { ADV_TERRA, ADV_URANUS, ADV_SATURN },
			{ ADV_VENUS, ADV_TERRA, ADV_MERCURY }, { ADV_JUPITER, ADV_NEPTUNE, ADV_PLUTO },
	...
	static {
		ADVANCEMENTS = new Advancement[][] { { // Vampire Killer
				new AdvDodge(), new AdvDodge2(),

				new AdvHolyBible(), new AdvHolyWater(), new AdvStopwatch(), new AdvCross(), new AdvFist(),
				new AdvCrystal(), new AdvAirDash(), new AdvBackflip(), new AdvSlideKick(), new AdvWarpDash(),

				new AdvItemBreak(), new AdvSoulBlast(), new AdvSoulFlame(), new AdvSoulIce(), new AdvSoulSaint(),
				new AdvSoulWind() },
				{ new AdvBatMorph(), new AdvBatMorph2(), new AdvDarkMetamorphosis(), new AdvFlamesShot(),
						new AdvLavaShot(), new AdvMystMorph(), new AdvMystMorph2(), new AdvShadeTeleport(),
						new AdvSoulsStrike(), new AdvSummonSoul(), new AdvWolfMorph(), new AdvWolfMorph2(),
						new AdvMorph() },
				{ new AdvDodge(), new AdvDodge2(), new AdvCure(), new AdvEnchant(), new AdvEnergyShield(),
						new AdvLight(), new AdvMindblast(), new AdvMindlock(), new AdvRecover(), new AdvTeleport(),
						new AdvMajorJinx() },
				{ new AdvDodge(), new AdvDodge2(), new AdvBirdsEgg(), new AdvCatSoul(), new AdvDragonFire(),
						new AdvInvokeBird(), new AdvInvokeCat(), new AdvInvokeDragon(), new AdvInvokeEagle(),
						new AdvInvokeTiger(), new AdvInvokeTortoise(), new AdvInvokeTurtle(), new AdvManipulate(),
						new AdvTame(), new AdvConfidence(), new AdvSoulForge(), new AdvKindSoul() },
				{ new AdvDodge(), new AdvDodge2(), new AdvBearMorph(), new AdvBeastMorph(), new AdvClawAssault(),
						new AdvCompleteControl(), new AdvDemonMorph(), new AdvEnergyScythe(), new AdvImpactBlow(),
						new AdvPowerBlow2(), new AdvImpactBlow3(), new AdvRegen(), new AdvSelfControl() },
				{
	...
	public void morph(String morphID, int count, boolean smallMorph, boolean bigMorph, int morphStrength,
			int loseMindChance) {
		deMorph();
	...
	public boolean isMorphed() {
		return hasCounter(Consts.C_LUPINEMORPH) || hasCounter(Consts.C_BEARMORPH) || hasCounter(Consts.C_BEASTMORPH)
				...
				|| hasCounter(Consts.C_BATMORPH) || hasCounter(Consts.C_BATMORPH2) || hasCounter(Consts.C_WOLFMORPH)
	...
	public int getPunchDamage() {
		...
		if (hasCounter(Consts.C_BEASTMORPH)) {
			...
			punchDamage = (int) (10 + 2 * getAttack() + Math.ceil(1.3d * punchDamage));
		} else if (hasCounter(Consts.C_POWERBLOW2)) {
			punchDamage = (int) (16 + 2 * getAttack() + Math.ceil(1.5d * punchDamage));
		} else if (hasCounter(Consts.C_POWERBLOW3)) {
			punchDamage = (int) (25 + 3 * getAttack() + Math.ceil(1.7d * punchDamage));
	...
	public int getWeaponAttack() {
		...
		if (weapon != null)
			if (getPlayerClass() == Player.CLASS_VAMPIREKILLER) {
				return (int) (multiplier * (weaponSkill(weapon.getDefinition().getWeaponCategory())
						+ (int) Math.round(getAttack() * (weapon.getAttack() / 2.0D))));
			} else
				return (int) (multiplier * (weapon.getAttack() + weaponSkill(weapon.getDefinition().getWeaponCategory())
						+ getAttack() + (weapon.hasCounter(Consts.C_WEAPON_ENCHANTMENT) ? 2 : 0)));
		else
	...
	public String getCustomMessage(String messageID) {
		return customMessages.get(messageID);
	}
