...
public class GFXEffects {
	private final ArrayList<Position> SFX_BIBLE_STEPS = new ArrayList<>(10);
	...
	private BufferedImage createSlashFrame(BufferedImage tileset, int baseX, int baseY, int baseWidth, int baseHeight) {
		...
		try {
			return ImageUtils.crearImagen(tileset, baseX * effectsScale, baseY * effectsScale, baseWidth * effectsScale,
	...
	private BufferedImage[] load(int frames, int xpos, int ypos) {
		...
		for (int x = 0; x < frames; x++) {
			try {
				ret[x] = ImageUtils.crearImagen(IMG_EFFECTS, (xpos + x) * 32 * effectsScale, ypos * 32 * effectsScale,
						32 * effectsScale, 32 * effectsScale);
			} catch (Exception e) {
	...
	private BufferedImage[] loadSame(BufferedImage file, int times, int xpos, int ypos) {
		...
		for (int x = 0; x < times; x++) {
			try {
				ret[x] = ImageUtils.crearImagen(file, xpos * 32 * effectsScale, ypos * 32 * effectsScale,
						32 * effectsScale, 32 * effectsScale);
			} catch (Exception e) {
				Game.crash("Error loading effect", e);
			}
	...
	private BufferedImage[] loadMaterialize() {
		...
		try {
			return new BufferedImage[] {
					...
					ImageUtils.crearImagen(IMG_EFFECTS, 44 * effectsScale, 485 * effectsScale, 40 * effectsScale,
							59 * effectsScale),
					ImageUtils.crearImagen(IMG_EFFECTS, 88 * effectsScale, 485 * effectsScale, 40 * effectsScale,
							59 * effectsScale) };
		} catch (Exception e) {
			Game.crash("Error loading effect", e);
		}
	...
	private BufferedImage[] loadVanish() {
		...
		try {
			return new BufferedImage[] {
					ImageUtils.crearImagen(IMG_EFFECTS, 129 * effectsScale, 485 * effectsScale, 40 * effectsScale,
							59 * effectsScale),
					ImageUtils.crearImagen(IMG_EFFECTS, 172 * effectsScale, 485 * effectsScale, 40 * effectsScale,
							59 * effectsScale),
					ImageUtils.crearImagen(IMG_EFFECTS, 215 * effectsScale, 485 * effectsScale, 40 * effectsScale,
							59 * effectsScale) };
		} catch (Exception e) {
			Game.crash("Error loading effect", e);
		}
	...
	protected void LoadEffects() {
		effects = new GFXEffect[] {
				...
				new GFXSplashEffect("SFX_MANDRAGORA_SCREAM",
						loadSame(configuration.getImageConfiguration().MonstersImage, MandragoraScream.SCREAM_RANGE, 8,
								6),
						120, configuration),
				new GFXSplashEffect("SFX_HOLY_RAINSPLASH",
						new Image[] { load1(4, 0)[0], load1(5, 0)[0], load1(4, 0)[0], load1(5, 0)[0] }, 50,
...
//			En Attack.java : 128 "SFX_"+weaponDef.getID()
				...
				// Weapons
				/*
				 * new GFXBeamMissileEffect("SFX_WP_AGUEN", "/\\", ConsoleSystemInterface.BLUE,
				 * 46),
				 */
