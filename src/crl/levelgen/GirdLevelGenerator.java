...
public class GirdLevelGenerator extends LevelGenerator {
	...
	public Level generateLevel(Position size, int startIndex, int endIndex, int roomWidth, int roomHeight,
		...
		Debug.enterMethod(this, "generateLevel", "size" + size + " startindex " + startIndex + " endIndex " + endIndex
	...
	private boolean connected() {
		...
		do {
			now = stack.pop();
			if (now.x > 0 && goneThru[now.x - 1][now.y]) {
				stack.push(new Position(now.x - 1, now.y));
				goneThru[now.x - 1][now.y] = false;
			}
			if (now.y > 0 && goneThru[now.x][now.y - 1]) {
				stack.push(new Position(now.x, now.y - 1));
				goneThru[now.x][now.y - 1] = false;
			}
			if (now.x < getWidth() - 1 && goneThru[now.x + 1][now.y]) {
				stack.push(new Position(now.x + 1, now.y));
				goneThru[now.x + 1][now.y] = false;
			}
			if (now.y < getHeight() - 1 && goneThru[now.x][now.y + 1]) {
				stack.push(new Position(now.x, now.y + 1));
				goneThru[now.x][now.y + 1] = false;
			}
			if (now.equals(end)) {
	...
	private void carveRoom(Cell[][][] cells, int px, int py) throws CRLException {
		...
		for (int x = 0; x < roomWidth; x++)
			for (int y = 0; y < roomHeight; y++)
				cells[0][x + px * roomWidth][y + py * roomHeight] = MapCellFactory.getMapCellFactory()
						.getMapCell(baseFloor);
		cells[0][px * roomWidth][py * roomHeight] = MapCellFactory.getMapCellFactory().getMapCell(baseWall);
		cells[0][(px + 1) * roomWidth - 1][(py + 1) * roomHeight - 1] = MapCellFactory.getMapCellFactory()
				.getMapCell(baseWall);
