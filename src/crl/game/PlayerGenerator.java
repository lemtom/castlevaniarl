...
public abstract class PlayerGenerator {
	...
	public Player getPlayer(String name, int sex, int choice) {
		...
		case Player.CLASS_RENEGADE:
			...
			player.setShield(ItemFactory.getItemFactory().createShield(Util.randomElementOf(RENEGADE_SHIELDS),
					Util.randomElementOf(ARMOR_MATERIALS)));
			player.setSecondaryWeapon(
					ItemFactory.getItemFactory().createWeapon("BASELARD", Util.randomElementOf(WEAPON_MATERIALS)));
		...
		case Player.CLASS_KNIGHT:
			...
			String[] randoms = new String[] { ItemDefinition.CAT_UNARMED, ItemDefinition.CAT_DAGGERS,
					ItemDefinition.CAT_SWORDS, ItemDefinition.CAT_SPEARS, ItemDefinition.CAT_WHIPS,
					ItemDefinition.CAT_MACES, ItemDefinition.CAT_STAVES, ItemDefinition.CAT_RINGS,
					ItemDefinition.CAT_PROJECTILES, ItemDefinition.CAT_BOWS, ItemDefinition.CAT_PISTOLS,
			...
			player.setAdvancementLevels(ADVANCEMENT_LEVELS_NORMAL);
			player.setShield(ItemFactory.getItemFactory().createShield(Util.randomElementOf(KNIGHT_SHIELDS),
					Util.randomElementOf(ARMOR_MATERIALS)));
			player.putCustomMessage("INTRO_1",
		...
		player.setPlot(ScriptUtil.replace(marks, replacements, Util.randomElementOf(plots)),
				ScriptUtil.replace(marks, replacements, getClassStartEquipmentDescription(player.getPlayerClass())));
		player.setDescription(ScriptUtil.replace(marks, replacements, Util.randomElementOf(descriptions)));
		if (initWeapons != null)
			player.setWeapon(
					itf.createWeapon(Util.randomElementOf(initWeapons), Util.randomElementOf(WEAPON_MATERIALS)));
	...
	protected String[] CLASS_DESCRIPTIONS = new String[] {
			...
			"A Vampire turned to the side of light, his will is to clean the world "
					+ "from the dark influence of the count. Able with weapons and wielder of " + "vampiric skills.",
			...
			"Able to manipulate spirits and bring them in a physical form to bring havoc down to " + "its enemies.",
	...
	protected String[] CLASS_APPEARANCES = new String[] { "VKILLER", "RENEGADE", "VANQUISHER", "INVOKER", "MANBEAST",
			"KNIGHT" };

	protected String[] CLASS_NAMES = new String[] { "Vampire Killer", "Darkness Renegade", "Vanquisher Wizard",
			"Souls' Master", "Manbeast", "Knight of the Order" };
