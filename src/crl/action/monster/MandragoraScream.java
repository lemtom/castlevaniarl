...
public class MandragoraScream extends Action {
	...
	public void execute() {
		if (performer.getFlag("MANDRAGORA_PULLED")) {
			...
			performer.getLevel().addEffect(
					EffectFactory.getSingleton().createLocatedEffect(performer.getPosition(), "SFX_MANDRAGORA_SCREAM"));

			loopOverMonsters();

			if (Position.flatDistance(performer.getPosition(),
					performer.getLevel().getPlayer().getPosition()) < SCREAM_RANGE) {
				performer.getLevel().getPlayer().damage("You hear the mandragora scream!", (Monster) performer,
	...
	private void loopOverMonsters() {
		...
		for (int i = 0; i < monsters.size(); i++) {
			...
			if (monster == performer)
				continue;
			if (evaluateMonster(monster)) {
				StringBuilder messages = new StringBuilder(
						"The " + monster.getDescription() + " hears the mandragora scream!");
	...
	private boolean evaluateMonster(Monster monster) {
		return Position.flatDistance(performer.getPosition(), monster.getPosition()) < SCREAM_RANGE
				&& !(monster instanceof NPC || monster instanceof Hostage);
	}
}
