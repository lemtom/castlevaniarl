...
public class ShadowFlare extends Action {
	...
	public void execute() {
		...
		if (targetDirection == Action.UP || targetDirection == Action.DOWN) {
			...
			runner3 = Position.add(performer.getPosition(), new Position(randy, 0));
		} else {
			runner2 = Position.add(performer.getPosition(), new Position(0, -1 * randy));
			runner3 = Position.add(performer.getPosition(), new Position(0, randy));
		}

		Position directionVar = Action.directionToVariation(targetDirection);

		drawEffect(EffectFactory.getSingleton().createDirectedEffect(runner1, Position.add(runner1, directionVar),
				"SFX_SHADOW_FLARE", 10));
		drawEffect(EffectFactory.getSingleton().createDirectedEffect(runner2, Position.add(runner2, directionVar),
				"SFX_SHADOW_FLARE", 10));
		drawEffect(EffectFactory.getSingleton().createDirectedEffect(runner3, Position.add(runner3, directionVar),
				"SFX_SHADOW_FLARE", 10));
		for (int i = 0; i < 10; i++) {
			hit(runner1);
			hit(runner2);
