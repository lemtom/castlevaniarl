...
public class MonsterWalk extends Action {
	...
	public void execute() {
		...
		if (standing != null && standing.getEffectOnStep() != null) {
		...
		if (destinationFeature != null && destinationFeature.isSolid()) {
			if (aMonster.wasSeen())
				aLevel.addMessage(
						"The " + aMonster.getDescription() + " hits the " + destinationFeature.getDescription());
			destinationFeature.damage(aMonster);
		}
		if (canChangeDirection(aMonster, destinationCell)) {
			((BasicMonsterAI) aMonster.getSelector()).setChangeDirection(true);
		}
		if (evaluate(aMonster, destinationCell, destinationFeature)) {
			if (destinationMonster == null) {
				if (aMonster.isEthereal() || aMonster.isFlying() || currentCell == null
					...
					if (destinationCell.isShallowWater() || destinationCell.isWater()) {
						...
						handleMovement(aMonster, destinationPoint, aLevel);
					}
				}
			} else {
	...
	private void handleMovement(Monster aMonster, Position destinationPoint, Level aLevel) {
		if (aLevel.getPlayer().getPosition().equals(destinationPoint)) {
			if (aLevel.getPlayer().getStandingHeight() == aMonster.getStandingHeight()) {
				...
				aLevel.getPlayer().damage("You are hit by the " + aMonster.getDescription() + "!", aMonster,
				...
				if (aLevel.getSmartFeature(destinationPoint) != null) {
					...
					if (sf.getDamageOnStep() > 0) {
						StringBuilder buff = new StringBuilder(
								"The " + aMonster.getDescription() + " is injured by the " + sf.getDescription());
	...
	private boolean evaluate(Monster aMonster, Cell destinationCell, Feature destinationFeature) {
		return evaluateDestinationCell(aMonster, destinationCell)
				&& (!(destinationFeature != null && destinationFeature.isSolid())
						&& (aMonster.isEthereal() || !destinationCell.isSolid()));
	}

	private boolean evaluateDestinationCell(Monster aMonster, Cell destinationCell) {
		return destinationCell != null && (!destinationCell.isEthereal()
				|| (destinationCell.isEthereal() && (aMonster.isEthereal() || aMonster.isFlying())));
	}
