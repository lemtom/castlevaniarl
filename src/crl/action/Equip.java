		...
		case 2:
			...
			if (targetItem.isTwoHanded()) {
				...
				if (currentShield != null) {
					if (!player.canCarry()) {
						performer.getLevel().addMessage("You can't store your " + currentShield.getDescription()
			...
			if (currentWeapon != null) {
				if (currentSecondaryWeapon != null) {
					if (!player.canCarry()) {
						performer.getLevel()
								.addMessage("You are unable to store your " + currentSecondaryWeapon.getDescription());
						return;
					} else {
						player.setSecondaryWeapon(currentWeapon);
						performer.getLevel()
								.addMessage("You put your " + currentWeapon.getDescription() + " in your belt.");
