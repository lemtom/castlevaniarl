...
public class Use extends Action {
	...
	public void execute() {
		...
		if (def.getID().equals("OXY_HERB")) {
			handleOxyHerb(aPlayer);
			return;
		}

		if (def.getID().startsWith("ART_CARD_")) {
			if (aPlayer.getLevel().getMapCell(aPlayer.getPosition()).getID().equals("WEIRD_MACHINE")) {
				handleArtCard(aPlayer, def);
				return;
			} else {
				performer.getLevel()
						.addMessage("You raise the " + targetItem.getDescription() + " up high! Nothing happens...");
				return;
			}
		}
	...
	private void handleOxyHerb(Player aPlayer) {
	...
	private void handleArtCard(Player aPlayer, ItemDefinition def) {
		aPlayer.getLevel().addMessage("You insert the card into the machine!");
		aPlayer.setFlag("HAS_" + def.getID(), true);
		if (aPlayer.getFlag("HAS_ART_CARD_SOL") && aPlayer.getFlag("HAS_ART_CARD_MOONS")
				&& aPlayer.getFlag("HAS_ART_CARD_DEATH") && aPlayer.getFlag("HAS_ART_CARD_LOVE")) {
			aPlayer.getLevel()
					.addMessage("The machine opens. A wooden music box plays a mellow melody. You take the jukebox");
			aPlayer.addItem(ItemFactory.getItemFactory().createItem("JUKEBOX"));
		}
		aPlayer.reduceQuantityOf(targetItem);
	}

	private void addMessage(Player aPlayer, String[] effect, int cmd) {
	...
	private static final String[][] clues = new String[][] { { // Lv 0 clues
			"They say stairs are the safest place if you dont want to be moved",
			"The thorn bracelet exchanges pain for power", "Every 100 years, the castle rises as its lord revives",
			...
			"The flame lasts less than the powder, but it is more powerful" }, // Lv 1 clues
			{ "..Secret treasures raised when I step on the spots..",
					...
					".. and the mystic axe, able to cut throw flying and spectres.." }, // Lv 2 clues
