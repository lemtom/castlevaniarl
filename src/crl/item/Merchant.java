...
public class Merchant extends NPC {
	...
	public List<MenuItem> getMerchandiseFor(Player player) {
	...
	/*
	 * private void refreshMerchandise(Player player){ merchandises = new
	 * ArrayList(); ItemDefinition[] defs = Items.getItemDefinitions(); //int
	 * itemNumber = Util.rand(6,12); int itemNumber = Util.rand(6,12); int items =
	 * 0; int tries = 0; while (items < itemNumber){ tries++; if (tries > 200)
	 * break; ItemDefinition trying = defs[Util.rand(0, defs.length - 1)]; if
	...
	public void refreshMerchandise(Player player) {
		if (player.getPlayerClass() == Player.CLASS_VAMPIREKILLER
				&& (merchandiseType == ItemDefinition.SHOP_WEAPONS || merchandiseType == ItemDefinition.SHOP_ARMOR))
			return;
		...
		while (items < itemNumber) {
			...
			if (!Util.chance(def.getShopChance()))
				continue;
			if (idsList.contains(def.getID()))
				continue;
			items++;
			Item item = new Item(def);
			idsList.add(def.getID());
			if (def.isUnique())
				inventory.add(item);
				...
				if (!def.isFixedMaterial())
					ItemFactory.getItemFactory().setMaterial(item, level.getLevelNumber(),
							ItemFactory.MOD_ARMOR_MATERIAL);
				if (Util.chance(10))
	...
	private static final String[] merchantNames = new String[] { "Kaleth", "Adam", "Invenior", "Dimitri", "Merdotios",
			"Richard", "Tommy", "Valentina", "Astrith", "Julieth", "Jazeth", "Juran", "Camilla", "Emer" };
